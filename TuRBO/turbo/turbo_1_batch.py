...
# This is fixed_seed version for replicable results.
# Place this file in ~/TuRBO/turbo/turbo_1.py
...
class Turbo1B:
    ...
    def __init__(
        ...
        print("Start TuRBO with seed = ", seed)
    ...
    def optimize(self):
        ...
        while self.n_evals < self.max_evals:
            ...
            for i in range(self.n_init//self.batch_size):
                fX_init_batch = np.array(self.f(X_init[i*self.batch_size:(i+1)*self.batch_size,:])).reshape(self.batch_size,1) # change
                fX_init = np.concatenate((fX_init,fX_init_batch),axis=0)
                print(f"Initialized {(i+1)*self.batch_size} points!")
            f_end = time.time()
            ...
            if self.verbose:
                ...
                sys.stdout.flush()
            print("Time taken for,",self.n_init,"points excluding f(x)",time.time()-start_time-(f_end-f_start))
            ...
            while self.n_evals < self.max_evals and self.length >= self.length_min:
                ...
                # Evaluate batch
                f_start = time.time()
                fX_next = np.array(self.f(X_next)).reshape(self.batch_size,1) # change
                f_end = time.time()
                ...
                print('Time spent at iteration excluding f(x):',self.n_evals,'is',time.time()-start_time-(f_end - f_start))
